# %%
# Ordinal regression via Frank & Hall method (sklearn only)
# CN (0) < MCI (1) < Dementia (2) — natural ordering
# Train K-1 binary classifiers: each predicts P(class > k)
# Combine: P(class == k) = P(y > k-1) - P(y > k)
import numpy as np
import pandas as pd
...
# %%
# Load data, drop NaN, encode diagnosis as ordinal int
...
# %%
# Frank & Hall ordinal regression
class OrdinalLogisticRegression:
    """Ordinal regression via K-1 binary classifiers (Frank & Hall 2001)."""
    ...
    def predict_proba(self, X):
        # P(y > k) for k = 0..K-2
        probs_gt = np.column_stack([clf.predict_proba(X)[:, 1] for clf in self.clfs])
        # P(y == k):
        #   P(y == 0) = 1 - P(y > 0)
        ...
        probs[:, -1] = probs_gt[:, -1]
        # Clip negatives from ill-ordered probabilities
...
ord_model = OrdinalLogisticRegression(n_classes=len(DX_ORDER))
ord_model.fit(X_train, y_train.values)

# %%
# Evaluate on test set
y_pred = ord_model.predict(X_test)

print("=== Ordinal Regression Performance ===")
...
for train_idx, test_idx in kf.split(X_scaled):
    m = OrdinalLogisticRegression(n_classes=len(DX_ORDER))
    m.fit(X_scaled[train_idx], y.values[train_idx])
    acc_cv.append(m.score(X_scaled[test_idx], y.values[test_idx]))
print(f"\n5-Fold CV Accuracy: {np.mean(acc_cv):.4f} ± {np.std(acc_cv):.4f}")
...
ax.set_title("Confusion Matrix — Ordinal Regression")
...
# %%
# Coefficients for each threshold classifier
for k, clf in enumerate(ord_model.clfs):
    print(f"=== Classifier {k}: P(diagnosis > {DX_ORDER[k]}) ===")
...
# Save model
import joblib
joblib.dump(ord_model, "ordinal_diagnosis_model.joblib")
joblib.dump(scaler, "ordinal_scaler.joblib")
print("Saved: ordinal_diagnosis_model.joblib, ordinal_scaler.joblib")

# %%
# Ordinal-aware stats: MAE and off-by-one accuracy
# (predicting MCI when true is Dementia is less wrong than predicting CN)
...
# %%
# Feature coefficient comparison across the K-1 classifiers
coef_df = pd.DataFrame(
    {f"P(>{DX_ORDER[k]})": clf.coef_[0] for k, clf in enumerate(ord_model.clfs)},
    index=X.columns,
)
print(coef_df)
...
# %%
# Predicted class probability distributions (how confident is the model?)
proba_test = ord_model.predict_proba(X_test)
...
# %%
# Normalized confusion matrix (rows sum to 1, i.e. recall per class)
